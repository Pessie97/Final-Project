$(document).ready(function() {
	...
	$("#email_form").submit(
		function(event) {
			...
			if(gender==""){
				$("#gender").next().text("This field is required.");
				isValid=false;
			}
			else {
				$("#gender").next().text("");
			...
			var buttonArray = [];
			buttonArray= $(":checkbox:checked");
			if (buttonArray.length == 0)
			{
				$("#swim").next().text("Select at least one");
				isValid=false;
			}
			else {
				$("#swim").next().text("");
			...
			// validate the check boxes	
			var button2Array = [];
			button2Array= $(":checkbox:checked");
			if (button2Array.length == 0)
			{
				$("#prof").next().text("Select at least one");
				isValid=false;
			}
			else {
				$("#prof").next().text("");
